...
Each lane runs **review → categorize → verify → dedupe** on its own findings the moment its reviewer finishes.  

A categorize agent first triages each finding — affirmations of correct behavior, diagnostics the compiler already reports, and claims that cannot be anchored to the patch are dropped before anyone spends a verification on them. A separate verification agent then adversarially checks what is left against the actual code, and a dedupe stage collapses the echoes.  
...
- `categorize_tokens_used` is a breakdown of tokens used by the categorize agents
...
- `agent_runs` entries each carry their own `tokens_used` breakdown per `role`:
  ...
  - `categorize` — a **per-finding** triage agent that runs before verification
