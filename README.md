...
```bash
# Pick the kind of local change to review from a list (on a terminal), then
# continue with that mode's own prompts; without a terminal this prints help
nickpit git
...
On GitLab, a re-review updates the review already on the MR instead of posting a second one. The default workflow has a "Published findings" lane next to the reviewers: `import-findings` (source `published-review`) imports the open findings of the published review into their own group, and `verify:published` verifies each one again against the current code, telling the verifier it may be outdated and skipping the diff-scope filter (leaving the diff proves nothing). The surviving ones go through merge (which de-duplicates them against the new findings) and finalize like any other finding. Then the `reconcile` step, before the verdict, decides how the run folds into the published review: published findings keep their identity, also when merge folds a new duplicate into one; one the verifier refuted is resolved with its remarks; one merge folded into a finding that no longer matches its thread is resolved as merged into that finding, which is posted as new; one dropped for any other reason stays open as published, since that proves nothing, and the verdict sees those as read-only context so its text cannot contradict open threads. Publishing works like a chat correction: the summary gets a history entry, findings whose visible text changed are edited in place, new findings open new threads, and a reply in the summary thread lists what was new, resolved, or updated. If a chat correction lands while the re-review runs, the re-review is replayed onto it. A run whose reviewers all failed leaves the published review alone. `import-findings` also imports findings files (`source: file`, with `findings_from`) into a named group that `verify:<group>` and merge treat like a reviewer's.
