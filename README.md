...
`--time-budget-scale` (or `time_budget_scale`) multiplies every absolute `max_seconds` in the spec — steps, their `mine_reasoning`/`compile_findings`/`nudge` subconfigs, and lane/pipeline/parallel group configs — without touching the file. `2` gives the whole review twice the time it declares, `0.5` half; the default `1` runs the spec as written. `weight` and `speedup_threshold` are deliberately left alone: both are relative to the parent budget, so they already move with it. Scaled values are clamped to at least one second and at most ten days. This is the knob for "let this review take longer" — raising an outer bound alone would not help, because each step's own `max_seconds` still applies. `--disable-workflow-time-budget` ignores every `time_budget` entry instead, and takes precedence over the scale.
