...
func TestListOpenPRs(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = w.Write([]byte(`[
			{"number":60,"title":"fix(llm): retry on 429","html_url":"https://codeberg.org/o/r/pulls/60",
			 "draft":false,"updated_at":"2026-09-01T10:00:00Z","user":{"login":"bob"},
			 "head":{"ref":"fix/retry"},"base":{"ref":"main"}},
			{"number":61,"title":"feat(review): cluster merge","html_url":"https://codeberg.org/o/r/pulls/61",
	...
	requests, err := NewClient(server.URL, "token").ListOpenPRs(context.Background(), "owner/repo")
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, want := range []string{"state=open", "sort=recentupdate", "limit=50"} {
	...
	if first.WebURL != "https://codeberg.org/o/r/pulls/61" {
