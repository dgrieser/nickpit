...
func (cs *carrierServer) start(t *testing.T) *Client {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/api/v1/repos/owner/repo/pulls/123/reviews":
			// One review with a body per entry, plus one review that only
			// carries the inline comments.
			reviews := append([]map[string]any{}, cs.reviews...)
			reviews = append(reviews, map[string]any{"id": 99, "body": "", "comments_count": len(cs.comments), "user": map[string]any{"login": cs.login}})
			write(w, reviews)
		case "/api/v1/repos/owner/repo/pulls/123/reviews/99/comments":
			write(w, cs.comments)
		case "/api/v1/repos/owner/repo/issues/123/comments":
			write(w, cs.issues)
		default:
...
func carrierReview(t *testing.T) (*model.ReviewResult, []string) {
	...
	result := &model.ReviewResult{
		...
		OverallExplanation: "forgejo carrier test",
		Repo:               "owner/repo",
		Identifier:         123,
...
func TestReviewResultsFailsWhenTokenUserIsUnknown(t *testing.T) {
	...
	// trusted or none: fail instead of guessing.
...
// Markers published by another account are refused by the strict reader and
// shown by the read-only one.
...
// A surface that cannot be read fails reassembly rather than looking like an
// absent review.
func TestReviewResultsFailsOnPartialRead(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch {
		case strings.HasSuffix(r.URL.Path, "/user"):
			_, _ = w.Write([]byte(`{"login":"mine"}`))
	...
	if _, err := NewAdapter(NewClient(server.URL, "token"), "").ReviewResults(context.Background(), "owner/repo", 17); err == nil {
		t.Fatal("a partial read must fail")
	}
}
