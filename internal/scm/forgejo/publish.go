...
// A marker-free line here avoids re-posting (and re-deduping) the verdict.
...
// PublishReview posts the review back to the pull request as a single review
// (event COMMENT): the overall verdict as the review body and one inline
...
func (a *Adapter) PublishReview(ctx context.Context, req model.ReviewRequest, result *model.ReviewResult) error {
	...
	// omitted because it would have pushed the comment past the size limit.
...
// (see the GitHub twin and reviewmd.IsStaleCarrierBody). The author check is
...
func (a *Adapter) pruneStaleCarriers(ctx context.Context, repo string, number int, currentReviewID string) {
	...
	// Reviews still referenced by a VISIBLE ref stub keep their carriers. Stubs
	// can live in review bodies and inline review comments too, so all three
	// comment surfaces feed the protected set. (A forged ref can at worst
	// RETAIN old carriers, never delete.)
	var bodies []string
...
// publishReview posts the summary + inline findings as one review. The
// create-review call is atomic, so if the server rejects an inline anchor the
// whole review fails; we then degrade gracefully — post the summary as a
// body-only review and each inline finding as a general comment — rather than
...
func (a *Adapter) publishReview(ctx context.Context, render reviewmd.Renderer, reviewsPath, issueCommentsPath, headSHA, summaryBody string, inline []inlineItem) error {
	...
	if !IsAPIError(err) || len(comments) == 0 {
...
func (a *Adapter) existingComments(ctx context.Context, repo string, number int) reviewmd.Priors {
	...
	found, _ := a.client.fetchSurfaces(ctx, repo, number, true)
	for _, review := range found.reviews {
