...
func (c *Client) listPRs(ctx context.Context, repo, state string) ([]model.OpenRequest, error) {
	path := fmt.Sprintf("/repos/%s/pulls?state=%s&sort=recentupdate", escapeRepo(repo), state)
	var response []openPRResponse
	if err := c.GetPaginated(ctx, path, &response); err != nil {
		return nil, err
	}
	...
	// sort parameter, with the number breaking ties for determinism.
