...
func TestShowTruncatesOversizedPatch(t *testing.T) {
	...
	runner.match = func(args []string) (string, bool) {
		...
		return "diff --git a/a.go b/a.go\n" + strings.Repeat("+line\n", maxGitCommitPatchBytes/6+10), true
	}
...
func TestShowFallsBackToMetadataLookupWhenEntriesAreCut(t *testing.T) {
	...
	runner.match = func(args []string) (string, bool) {
		...
		default:
			...
			return strings.Repeat("x", maxGitCommitPatchBytes+16), true
		}
	}
...
func TestShowBoundsPatchBytesReadFromGit(t *testing.T) {
	const oversized = maxGitCommitPatchBytes + 4096
	...
	wantPatchCalls := maxGitShowPatchBytes / maxGitCommitPatchBytes
	...
	remaining := maxGitShowPatchBytes
	for i, limit := range limits {
		if limit > maxGitCommitPatchBytes || limit > remaining {
			t.Fatalf("cap[%d] = %d, want <= min(%d, %d)", i, limit, maxGitCommitPatchBytes, remaining)
		}
		remaining -= limit
	}
...
func TestShowFirstParentFallbackRespectsRemainingBudget(t *testing.T) {
	...
	if total > maxGitShowPatchBytes {
		t.Fatalf("reads total %d bytes, want at most %d", total, maxGitShowPatchBytes)
	}
