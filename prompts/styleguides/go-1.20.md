...
and any number of times. They are not prefix/suffix removal — that is what
`TrimPrefix`/`TrimSuffix` (or `CutPrefix`/`CutSuffix`, 1.20+) are for.
...
between the check and the open. `os.Root` (1.24+) is the first standard-library
API without that race.
