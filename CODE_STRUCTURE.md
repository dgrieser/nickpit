...
- `internal/pick/list.go`: The picker's pure state and rendering — cursor, scroll window, AND-term filter, column fitting, per-column and per-row colours, the selected row's pastel highlight, and the full value of that row in the title line — driven by keys and returning lines, so its behaviour is testable without a terminal. Its palette is the 256-colour message palette of `internal/logging/progress.go`, mirrored in `tools/print_colors.sh`.
