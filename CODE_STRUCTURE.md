...
- `cmd/nickpit/main.go`: Main CLI entry point. Defines commands, flags, profile loading, workflow execution, local review modes, output selection, publishing, seed-finding handling, and post-review chat-session persistence.
- `cmd/nickpit/forge_cmd.go`: The platform-generic half of the CLI: the `<platform> <pr|mr>` command tree built once per registered forge (`--repo`/`--id`/`--url`/`--select`/`--publish`), the per-platform `--<mode>-token` and `--<mode>-base-url` persistent flags and the overrides they become, and the mode-to-platform lookups the rest of the CLI uses (checkout credentials, request sigil and labels).
...
- `cmd/nickpit/select.go`: Interactive target selection for the CLI: the selector policy every MR/PR-addressed command shares (`--url` exclusivity, repo inference, and when a missing `--id` defers to a picker), the mode menu of a bare `nickpit git` (Esc in the chosen mode's prompts leads back to it), the open-request, branch and commit pickers (branch refs folded per branch, resolving to the remote side for a base and the local one for a head), the per-row colouring, and the terminal detection plus test seam behind them.
...
- `internal/review/agent_budget.go`: Non-reviewer soft-deadline finalization, small-model reasoning summaries, and aggregate budget-stop tracking.
...
- `internal/git/history.go`: Commit history provider for the git_log/git_show tools and `nickpit inspect log|show`. Deepens a shallow checkout with the `HistoryAuth` host credentials the platforms derive (`forges.HistoryAuth`), sent only to the exact host each belongs to.
...
- `internal/scm/gitlab/forge.go`: GitLab as a `forge.Forge`: configurable base URL and the trusted host derived from it, MR URL parsing (any host), same-host remote detection, `oauth2` git credentials.
...
- `internal/projectcontext/projectcontext.go`: Loads and merges the project's own description of how it is deployed and used. Reads the reviewed repository's `.nickpit/project.yaml` through `model.BaseFileSource` (always the base revision, so a fork cannot author it) and resolves operator entries from the profile or `--project-context`. Strict YAML decode (unknown keys are an error), 16 KiB cap; a malformed repo file warns, a malformed operator entry fails the run.
