...
func newRootCmd() *cobra.Command {
	...
	root.PersistentFlags().Var(newTrackedFloatValue(&cli.timeBudgetScale, &cli.timeBudgetScaleSet), "time-budget-scale", "Multiply every time_budget max_seconds in the workflow spec (2 = twice as much time, 0.5 = half)")
	...
	for _, f := range forges.All {
		forgeCmd := cli.newForgeCmd(f)
		if f.Mode() == model.ModeGitLab {
			forgeCmd.AddCommand(cli.newGitLabServeCmd())
			forgeCmd.AddCommand(cli.newGitLabTemplatesCmd())
		}
...
func (a *app) loadProfile() (string, config.Profile, error) {
	...
	var timeBudgetScale *float64
	if a.timeBudgetScaleSet {
		timeBudgetScale = &a.timeBudgetScale
	}
	...
	forgeTokens, forgeBaseURLs := a.forgeOverrides()
	cfg, profile, err := config.Load(a.configPath, config.Overrides{
		...
		TimeBudgetScale:           timeBudgetScale,
		Workdir:                   a.workDir,
		ForgeTokens:               forgeTokens,
		ForgeBaseURLs:             forgeBaseURLs,
	})
...
func (a *app) inspectHistory(ctx context.Context) (string, config.Profile, git.History, error) {
	...
	return repoRoot, profile, git.NewExecHistory(forges.HistoryAuth(profile)), nil
}
