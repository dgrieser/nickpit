...
func TestSeedFindingsSkipsParallelReviewersToSequentialConsumer(t *testing.T) {
	// Parallel groups hold only non-consuming review steps; seeding must skip
	// them and land on the sequential consuming step.
	spec := workflow.Spec{Version: 1, Steps: []workflow.StepEntry{
		{Parallel: []workflow.StepEntry{{Type: workflow.StepReviewPrefix + "security"}}},
		{Type: workflow.StepMerge},
	}}
	...
	if got := spec.Steps[1].FindingsFrom; len(got) != 1 || got[0] != "a.json" {
		t.Fatalf("merge not seeded: %v", got)
	}
}
